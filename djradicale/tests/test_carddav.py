...
<multistatus xmlns="DAV:"><response><href>{path}</href><status>HTTP/1.1 200 OK</status></response></multistatus>""",
    ...
    def setUp(self):
        ...
        DBProperties.objects.create(
            path="user/addressbook.vcf", text='{"tag": "VADDRESSBOOK"}'
        )
        self.client = DAVClient()

    def propfind_1_anonymous(self):
        path = reverse(
            "djradicale:application",
            kwargs={
        ...
        response = self.client.propfind(
            path,
            data=self.DATA_CASES["PROPFIND_1"]["request"].format(
                **{
                    "path": path,
                }
            ),
    ...
    def propfind_2(self):
        ...
        response = self.client.propfind(
            path,
            data=self.DATA_CASES["PROPFIND_2"]["request"].format(
                **{
                    "path": path,
                }
            ),
    ...
    def report_empty(self):
        ...
        response = self.client.report(
            path,
            data=self.DATA_CASES["REPORT_EMPTY"]["request"].format(
                **{
                    "path": path,
                }
            ),
    ...
    def report(self, fn, etag):
        ...
        response = self.client.report(
            path,
            data=self.DATA_CASES["REPORT"]["request"].format(
                **{
                    "path": path,
                }
            ),
        ...
        self.assertEqual(
            ...
            self.DATA_CASES["REPORT"]["response"]
            .format(
                **{
                    "path": path,
                    "fn": fn,
    ...
    def put(self, fn):
        ...
        response = self.client.put(
            path,
            data=self.DATA_CASES["PUT"]["request"].format(
                **{
                    "path": path,
                    "fn": fn,
                }
    ...
    # For an update to an existing element, it [seems](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Methods/PUT) that 204 is a valid response code
    def put_update(self, fn):
        path = reverse(
            "djradicale:application",
            kwargs={
                "url": "user/addressbook.vcf/test.vcf",
    ...
    def test_everything(self):
        ...
        self.report(
            fn="John Smith",
            etag="9227dd9db5e16cff639191598273c84b20c8a53719e11a200be855c661e44799",
        )
        # update
        self.put_update(fn="J.S.")
        self.get(fn="J.S.")
        self.report(
            fn="J.S.",
            etag="1ab6a4dd3221a09b3fe981f645d5f71f0e9dbcdff2a7343e2f498546b806dc9f",
        )
        self.delete()
        self.report_empty()
